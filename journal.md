...
## 16 novembre:
- J'ai compris ce que je n'avais pas réussi à faire pour la séance dernière, et j'ai pu corriger mon script et partir de là pour transformer la sortie du script en tableau html. Pour ne pas avoir à entrer les données unes par unes à la main dans un nouveau script html, il fallait partir du script bash et utiliser la boucle utilisée pour créer le ficher tsv. J'ai donc utilisé la fonction `echo` et la redirection pour écrire du html dans le script bash. J'ai commencé par définir l'en-tête, avec `echo` puis la syntaxe html, avec `<html>`, `<table>`, `<head>`, `<th>` et `<tr>` ainsi que les balises fermantes qui correspondent (sauf `</table>` et `</html>`, que j'utilise tout à la fin du script).
Dans la boucle `while`, après avoir défini les variables qui donnent le code http, l'encodage, le nombre de mots et le numéro de ligne, j'ai utilisé `echo` pour remplir le tableau, dont je n'avais défini que l'en-tête jusque là. Pour remplir le tableau j'ai utilisé `<body>`, `<tr>` et des `<td>`, ainsi que les balises fermantes qui correspondent.Comme on est dans une boucle, j'ai utilisé les variables définies plus haut, qui changent à chaque tour de boucle, pour remplir le tableau. Je n'ai donc eu besoin de renseigner le contenu des lignes qu'une seule fois, et pas autant de fois qu'il y a d'urls. Ainsi, tant que le script lit une ligne dans le fichier urls/fr.txt, il ajoute une ligne avec les données correspondantes dans le tableau html.
À la fin de la boucle, je redirige la sortie du programme vers le fichier tableau-fr.html, puis une fois sortie de la boucle je ferme le tableau, grâce à `echo`et aux balises fermantes `</table>`et `</html>`.

- Avant d'arriver au bon résultat j'ai eu quelques problèmes, d'abord parce que j'avais écrit tout le script html dans la boucle `while`, et l'en-tête du tableau réapparaissait à chaque ligne du résultat. Puis j'ai changé et défini l'en-tête avant la boucle, mais je n'avais pas utilisé `<body>` et j'avais utilisé la balise fermante `</table>` dans la boucle, le tableau se fermait donc à chaque tour de boucle, et après le premier tour il n'avait plus d'en-tête, puisqu'elle avait été utilisée au premier tour. Le programme renvoyait donc un tableau avec l'en-tête attendue et la première ligne correctement renseignée, puis toutes les autres lignes les unes collées aux autres. J'ai fini par comprendre le problème et fermer le tableau après la boucle, ce qui a corrigé le problème et renvoyé un tableau html propre.

## 25 novembre:
- Création d'une page d'accueil et utilisation de Bulma: j'ai créé une page d'accueil simple, avec un petit paragraphe de présentation et une ligne qui présentait le tableau, avec un lien vers celui-ci. J'ai ensuite essayé de "styliser" les pages grâce au style Burma. Même si je n'en maîtrise pas énormément d'aspects, j'ai quand même réussi à utiliser quelques fonctions. J'ai utilisé la balise `<title>` pour donner un titre aux pages, et j'ai utilisé des classes pour que le titre ressorte et que le texte soit divisé en plusieurs paragraphes séparés (`class="title"` pour le titre et `class="block"` pour les paragraphes).

- Pour le tableau, j'ai utilisé `is-bordered`, `is-hoverable` et `is-striped` pour que les colonnes soient visibles et pour donner un aspect plus "ordonné" au tableau (pour qu'il ait plus une "tête" de tableau). J'ai également utilisé `class="is-primary"` et `has-text-uppercase` pour l'en-tête (pour que les noms des colonnes soient entourés de couleur et soient en majuscules). Un problème que j'ai eu, est que mettre simplement `has-text-uppercase` n'a pas suffit au script pour mettre l'en-tête en majuscules. Pour que ça fonctionne, j'ai dû "expliquer" la fonction au script, en utilisant `.has-text-uppercase {text-transform:uppercase;}`. Et pour les urls, j'ai utilisé `href` pour faire en sorte que le texte de l'url dans le tableau soit un lien sur lequel on peut cliquer, qui renvoie vers l'url en question.

- Je n'ai pas eu trop de problèmes à faire cet exercice, mais je sais que mes résultats sont assez minimalistes, et que j'ai encore beaucoup de styles et de fonctions à découvrir.
